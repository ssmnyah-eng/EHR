...
// GET /api/bookings?year=2026&month=8  -> { unavailable: ["2026-08-02", ...] }
// GET /api/bookings                    -> { anyAvailability: boolean }
export async function GET(req: NextRequest) {
  const year = req.nextUrl.searchParams.get("year");
  const month = req.nextUrl.searchParams.get("month");
  if (year && month) {
    return NextResponse.json({
      unavailable: await unavailableDatesForMonth(Number(year), Number(month)),
    });
  }
  return NextResponse.json({ anyAvailability: await hasAnyAvailability() });
}

// POST /api/bookings, holds the day (one job per day), then either returns a
// Stripe Checkout URL (when STRIPE_SECRET_KEY is configured) or, in demo
// mode, confirms immediately. A booking only becomes "confirmed", and the
// confirmation email only sends, after payment succeeds.
export async function POST(req: NextRequest) {
  ...
  if (stripeKey) {
    ...
    // booking; abandoning checkout releases the held day via cancel_url.
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "Payment couldn't be started, the date was not held. Please try again." },
        { status: 502 }
      );
    }
